...
function mapItems(items: (Parser.Item & CustomItem)[], source: NewsSource): NewsItem[] {
  return (items || [])
    ...
    .map((item, index) => {
      ...
      return {
        // Use the normalized permalink as the stable ID.
        // GUIDs are often regenerated by CMSs on article edits, causing seen
        // articles to reappear. The permalink is much more stable.
        id: (() => {
          const link = item.link || '';
          try {
            const u = new URL(link);
            // host + path without trailing slash, ignoring protocol & query params
            return u.host + u.pathname.replace(/\/$/, '');
          } catch {
            return item.guid || link || `${source.name}-${index}`;
          }
        })(),
